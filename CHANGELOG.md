...
## [0.1.3-beta.7] - 2026-08-25

### AI 打回返工闭环与提示词升级

- 修复多个资源图被 AI 同时打回时只重做第一项的问题；每个对象独立保存审核证据并创建下一代生成任务。
- 打回理由、阻断项和改进建议会进入下一次文本、图片、3D、视频或成片返工，并保留未被批评的批准内容。
- 审核和各生成阶段的连续返工上限可配置；只有同一对象达到上限、预算/授权不足或外发结果不明确时才交给人工。
- 后台与浏览器推进器采用语义进度退避、租约和幂等 action，刷新、重启和双 runner 竞争不会丢任务或重复提交。
- 18 项默认生产提示词统一补齐目标、输出契约、正反例与自检；角色、场景、道具资源图明确为恰好四格并使用类型化排除词。
- 真实卡住草稿在只读数据库副本上完成零付费恢复验收，原数据库保持不变。

## [0.1.3-beta.6] - 2026-08-24

### 实时配置绑定

- 新的图片或视频重试在派发前读取最新配置与模型；已接单任务继续使用原提交快照轮询和下载。
- 保留智能路由与三 Key 路径，模型能力提示不阻止人工选择或参考包编辑。

## [0.1.3-beta.5] - 2026-08-19

### YinziAPI 智能路由一键配置修复

- 当前 Key 的 `/v1/models` 现在是可用模型的权威目录；一个智能路由 Key 返回的文本、生图和视频模型会按端点类型完整保存。
- 能力合同和公开价格改为非阻断增强。`/v1/model-capabilities` 暂时返回 500、401、404 或网络错误时，不再让已经通过鉴权的模型目录配置失败。
- 配置窗口可在保存前读取并展示“当前 Key 可用模型”；能力合同延迟只显示一次黄色提示，不再重复弹出全局错误。
- 缺少本地能力合同的新视频模型仍可手动选择。自动路由只有在没有已知兼容候选时，才会兜底使用该 Key 实际枚举到的未知模型，并把未知能力保留为提示而非伪造合同。
- 选中模型的动态能力会冻结到视频任务，并统一控制最终时长、参考角色、数量、文件大小和提交收据；标准合同没有本地 `route_profiles` 时不再被误判为不兼容。
- 智能路由只有在收到无任务 ID的确定性 `no eligible automatic route` 时，才会切换一次当前 Key 已验证的兼容模型；普通 5xx、超时和可能已接单的响应绝不自动重发。
- `/models` 鉴权失败、网络失败或确实没有视频模型仍会明确停止，避免把无效 Key 伪装成配置成功。

真实上游验收创建了两个本地 5 秒无参考媒体任务，共发生三次上游预创建尝试且均没有任务 ID：Seedance 2.5 两次返回智能路由无可用渠道，自动任务随后只切换一次 Grok，但其渠道返回上游 404。应用正确停止且没有额外重复提交；上游诊断见 `YINZI_SMART_ROUTER_VIDEO_DIAGNOSIS_20260820.md`。

## [0.1.3-beta.4] - 2026-08-17

### 视频提交对账与多参考能力修复

- 视频提交现在持久化区分 `not_sent`、`rejected`、`accepted` 和 `ambiguous`，本地重启、恢复与自动流程不会再把明确未送达和可能已送达混为一谈。
- 上游在创建任务前明确返回 HTTP 4xx 且没有任务 ID 时，会原子返还本地预留金额、一次尝试和对应视频秒数；同一提交重复恢复不会重复返还。
- 超时、断线、HTTP 5xx、异常 2xx 或 2xx 无任务 ID 统一标记为待对账，不自动退款、不自动重试，避免上游已经接单时再次付费提交。
- 修复 `seedance2.0 720p-pro-nv-nsp` 在银子 API 实际路由中退回 OpenAI 默认单图限制的问题；站点能力合同现按真实路由支持 9 张参考图、3 个参考音频和 5-15 秒时长。
- 错误提示会明确区分“请求未离开银子 API”“结果不确定需对账”和“上游已接单”，并保留请求号、模型、配置与参考包证据用于排查。

本版本的工程回归和发行验收不创建生图或视频任务，付费请求为 0。

## [0.1.3-beta.3] - 2026-08-16

### 视频配置绑定修复

- 修复镜头已经选择并保存视频配置后，实际派发仍沿用空配置或旧配置，并误报“未配置视频模型”的问题。
- 镜头级模型保存会同时持久化当前配置 ID；项目、镜头、创建、轮询、下载和历史恢复统一使用同一配置绑定。
- 用户明确选择的任意非空模型名会原样交给上游。本地模型目录与能力合同只提供自动路由、价格和参考包建议，不再阻止人工提交目录外模型。
- 旧任务可从视频生成记录或脱敏配置快照恢复绑定，清除旧人工接管提示后继续原模型重试；不会自动切换模型、调用文本 AI 或重复提交已有外部任务。
- 真正缺少启用的视频配置、URL 或 Key 时仍会给出明确的可操作提示。

本修复的本地回归与隔离验收不调用生图或视频接口，付费请求为 0。

## [0.1.3-beta.2] - 2026-08-16

### 银子 API 模型协议与小白配置

- 为当前特价视频分组的 10 个目标模型固化时长、分辨率、参考图/视频/音频数量、计费单位和自动路由资格，生产读回 10/10 匹配。
- 一键配置银子 API 只需 Base URL 和一个 Key；默认文本为 `gpt-5.6-sol`、生图为 `gpt-image-2`，视频留空时按镜头预计总价选择最便宜的兼容模型。
- 模型目录、价格和能力提示自动同步；未登记模型仍可手动输入和提交，能力提示不会限制人工选模或强制携带参考素材。
- 增加清晰的“模型与能力”编辑入口，可维护时长、参考媒体数量、媒体角色和自动路由授权。

本版本为 Windows x64 外部测试包，保持与 beta.1 相同的 appId 和用户数据目录，可直接升级且不删除作品或配置。

## [0.1.3-beta.1] - 2026-08-16

### 修复与优化

- 分离上游视频生成与本地取回状态，下载失败只恢复原任务，不重复创建付费视频。
- 增加镜头级跳过、恢复、拆分和补拍，修复重复处理镜头 1 以及无法推进下一镜头的问题。
- 资产图支持可配置并发、拖放批量上传、预校验和内容哈希去重。
- 视频配置支持按当前 Key 只读发现模型目录，并保证项目、阶段、镜头三级覆盖在实际派发中生效。
- 保留旧数据兼容升级、3D 导演台可选、首帧连续性、旁白字幕和全自动运行能力。

本版本为 Windows x64 验收包。macOS x64/arm64 构建与 GitHub Release 等待 Windows 验收后进行。
