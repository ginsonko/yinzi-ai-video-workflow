...
describe('release packaging contract', () => {
  ...
  it('pairs the local launcher, frontend proxy and canonical backend by runtime identity', () => {
    const launcher = fs.readFileSync(path.join(repositoryDir, 'start-ai-video-demo.ps1'), 'utf8');
    assert.match(launcher, /YINZI_WORKFLOW_CANONICAL/);
    assert.match(launcher, /Get-RuntimeIdentity/);
    assert.match(launcher, /Get-IdentityKey/);
    assert.match(launcher, /backendIdentity\.canonical -ne \$true/);
    assert.match(launcher, /Frontend proxy and backend runtime identities differ/);
    assert.match(launcher, /orchestration-modules/);
  });
});
