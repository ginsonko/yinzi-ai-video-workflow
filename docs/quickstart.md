...
Apple Silicon 下载文件名带 `mac-arm64` 的 DMG，Intel Mac 下载 `mac-x64`。优先使用 DMG 安装；未签名内测包首次打开时，按住 Control 点击应用并选择“打开”，不要关闭整个 Gatekeeper。若 macOS 显示“应用已损坏，无法打开”，先从 Release 下载 `SHA256SUMS.txt` 校验哈希，确认一致后再按《Mac 小白测试说明》中的“已损坏”分支处理。详细步骤见 [Mac 小白测试说明](../desktop/release-docs/Mac小白测试说明.md)。
