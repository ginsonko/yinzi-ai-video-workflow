    ...
    const req = mod.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          // 兼容标准 OpenAI 格式、推理模型及 relay 的简化响应。
          const content = responseTextFromJSON(json);
...
// Return the text portion of the common OpenAI-compatible response shapes.
// Keeping this in one place makes the explicit non-stream path behave like
// the streaming path without assuming that every relay uses exactly the same
// message/content representation.
function responseTextFromJSON(json) {
...
async function generateText(db, log, serviceType, userPrompt, systemPrompt, options = {}) {
  const {
    ...
    streamCallback = null,
    scene_key = null,
    // Keep the historical streaming default for callers that rely on
    // incremental progress; callers can explicitly opt into one bounded
    // JSON response for relays/models that do not emit SSE promptly.
    stream = true,
  } = options;
  ...
  log.info('AI generateText request', {
    url: url.slice(0, 60), model, max_tokens: finalMaxTokens ?? '(model default)', json_mode,
    stream: stream !== false,
  });
  let content;
  if (stream === false) {
    const timeoutMs = Math.min(600000, Math.max(30000,
      Number(options.timeout_ms ?? options.silence_timeout_ms ?? 180000) || 180000));
    const res = await postJSONNonStream(
      url,
    ...
    try { parsed = JSON.parse(res.raw || '{}'); } catch (_) {
      throw new Error('AI 非流式响应不是有效 JSON');
    }
    content = responseTextFromJSON(parsed) || res.body;
    log.info('AI non-stream response received', {
      model, status: res.status, text_length: content ? String(content).length : 0,
