...
function Get-RuntimeIdentity([string]$BaseUrl) {
  $payload = Invoke-RestMethod -Uri "$BaseUrl/api/v1/runtime-identity" -Method Get -TimeoutSec 5
  if (-not $payload.success -or -not $payload.data.schema -or $payload.data.orchestration_router -ne $true) {
    throw "工作流服务未声明当前编排能力：$BaseUrl"
  }
  return $payload.data
}
...
function Get-IdentityKey($Identity) {
  return @(
    ...
    [string]$Identity.app_version,
    [string]$Identity.source_revision,
    [string]$Identity.database.fingerprint,
    [string]$Identity.orchestration_router
  ) -join '|'
}
...
try {
  ...
  if ($null -ne $backendOwner) {
    ...
    $previousCanonical = $env:YINZI_WORKFLOW_CANONICAL
    try {
      $env:PORT = [string]$BackendPort
      # The launcher owns the canonical local runtime. MCP and the UI must
      # either pair with this identity or fail closed instead of guessing.
      $env:YINZI_WORKFLOW_CANONICAL = '1'
      ...
      $env:YINZI_WORKFLOW_CANONICAL = $previousCanonical
    }
  ...
  $backendIdentity = Get-RuntimeIdentity -BaseUrl $backendBaseUrl
  if ($backendIdentity.canonical -ne $true) {
    throw "Backend is healthy but is not marked canonical. Refusing to pair a non-canonical runtime."
  }
  $backendModules = Invoke-RestMethod -Uri "$backendBaseUrl/api/v1/orchestration-modules" -Method Get -TimeoutSec 5
  if (-not $backendModules.success -or -not $backendModules.data.items) {
    throw "Backend is healthy but does not expose orchestration modules."
  }
  ...
  $proxiedIdentity = Get-RuntimeIdentity -BaseUrl $frontendBaseUrl
  if ((Get-IdentityKey $proxiedIdentity) -ne (Get-IdentityKey $backendIdentity)) {
    throw "Frontend proxy and backend runtime identities differ; refusing to expose a split-brain UI."
  }
  ...
  Write-Host "  Runtime: canonical $($backendIdentity.runtime_id) / source $($backendIdentity.source_revision) / database $($backendIdentity.database.fingerprint)"
